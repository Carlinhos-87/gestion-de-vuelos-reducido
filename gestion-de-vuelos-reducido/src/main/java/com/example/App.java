...
public class App {
	public static void main(String[] args) {
		...
		Vuelo vuelo1 = Vuelo.builder().destino(Destino.PARIS).precio(new BigDecimal(150.0))
				.fechaSalida(LocalDate.now()).horaSalida(LocalTime.of(23, 30))
				.fechaLlegada(LocalDate.now().plusDays(1)).horaLlegada(LocalTime.of(12, 20)).numeroPlazas(3)
				.build();
...
//		vuelo1.getPasajeros().add(pasajero31);
		...
		Vuelo vueloConMasPasajeros = vuelos.stream()
				.max((v1,v2) -> Integer.valueOf(v1.getPasajeros().size())
						 .compareTo(v2.getPasajeros().size()))
				.orElse(null);
		...
		// Modernamente, año 2026. No hace falta el metodo collect al final de la tuberia
		...
		List<Vuelo> vuelosUltimos10DiasDelMes = vuelos.stream()
		...
		pasajerosPorGeneroYEdad = vuelos.stream()
				.flatMap(vuelo -> vuelo.getPasajeros().stream())
				   .collect(Collectors.groupingBy(Pasajero::genero,
						Collectors.groupingBy(pasajero -> ChronoUnit.YEARS
								.between(pasajero.fechaNacimiento(), LocalDate.now()))));
		
		// Ejercicio al margen del Pto 7. En lugar de tener una lista de pasajeros, quiero
		...
		nombresDePasajerosPorGeneroYEdadSinDuplicados = vuelos.stream()
				.flatMap(vuelo -> vuelo.getPasajeros().stream())
				   .collect(Collectors.groupingBy(Pasajero::genero,
						Collectors.groupingBy(pasajero -> ChronoUnit.YEARS
								.between(pasajero.fechaNacimiento(), LocalDate.now()), 
